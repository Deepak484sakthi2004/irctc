...
constructor(
    ...
    fun bookTicket(@Argument uid: Long,
        ...
        if (availSeats.isNotEmpty()) {
            // If seats are available, book the seat
            ...
            curSeat.seatStatus = SeatStatus.CONFIRMED
            ticketRepo.save(userTicket)
            return "Ticket booked with the id ${userTicket.ticketId} at ${userTicket.bookedAt} , to the seat id ${curSeat.seatId}"

        } else {
            if (trainInstance.racList.size < Train.MAX_RAC_SIZE) {
                val curRAC = RAC(
                    user = userInstance,
                    train = trainInstance,
                    status = SeatStatus.PENDING
                )
                trainInstance.racList.add(curRAC)
                    racRepo.save(curRAC)
    ...
    fun bookTicket1(@Argument uid: Long,
                   @Argument tid: String,
                   @Argument sStatid: Int,
                   @Argument endStatid: Int): String {
        val trainInstance = trainRepo.findById(tid).orElseThrow {
    ...
    fun cancelTicketBooking(@Argument ticketId: Long): String {
        ...
        ticketInstance.seat.seatStatus = SeatStatus.FREE
